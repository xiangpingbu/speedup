 ...
 */
package com.ecreditpal.maas.pmml.core;
...
import static com.ecreditpal.maas.pmml.container.obj.ModelNormalizeConf.NormType.WOE;
...
public class Normalizer {
    ...
    /**
     * By Xiangping
     * Compute the Special data for Woe Score.
     *
     * @param config
     * @param raw            input column value
     * @param isWeightedNorm if use weighted woe
     ...
     * @ColumnConfig info
     */
    private static Double woeSpecial(ColumnConfig config, String raw, boolean isWeightedNorm) {
        List<Double> woeBins = isWeightedNorm ? config.getBinWeightedWoe() : config.getBinCountWoe();
        int binIndex = CommonUtils.getBinNum(config, raw);
        if (binIndex == -1) {
            // 倒数第二个bin
            return woeBins.get(woeBins.size() - 2);
        } else {
            return woeBins.get(binIndex);
        }
    }
