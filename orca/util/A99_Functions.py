...
@author: Admin
"""
#df = pd.DataFrame({'a':[1,2,2,3,3,3],'b':[100,1000,2000,900,3000,200],'c':[20,21,'a',30,24,np.NaN]})
#df
...
def GetDFSummary(df):
    ...
    for i in range(len(df_name)):
        ...
        df_name.ix[i, 'coverage']=getColCoverage(df, col)
        df_name.ix[i, 'uniqueVal']=uniqueVal(df, col)
    ...
    print 'Total Variables: ', total_var, '; Numeric Variables: ', df_num.shape[1], '; Other Variables: ', total_var-df_num.shape[1]
    return df_name

        
def getColCoverage(df,col,null_value=[]):
    import pandas as pd
    import numpy as np
...
def uniqueVal(df,col,null_value=[]):
    import pandas as pd
    import numpy as np
